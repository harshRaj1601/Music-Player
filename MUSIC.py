...
def playlist(e):
    ...
    global l23
    filename = filedialog.askopenfilenames(parent=root, title="select a file",filetypes=(('music files', '.mp3'), ('all files', '.*')))
...
def button_():
    ...
    # btn6.place(x=200)
    btn5.place(x=520,y=388)
    btn2.place(x=200,y=388)
    btn3.place(x=300,y=388)
    btn4.place(x=420, y=388)
    volume(1)
...
def time(): 
    string = strftime('%I:%M %p')
    root.title("Music Player"+" "*70+string)
...
def main():
    global root,lbl2,ll,lbl4,red_frame,lbl3,sliderlbl,lbl9,b1,b2,defaultblue,b3,lbl91,myslider
